  ...
  navigation.items do |n|
    ...
    n.item :gamepatch, safe_join([material_symbol('extension'), t('admin.gamepatch.title')]), nil, if: -> { current_user.can?(:manage_settings) && !self_destruct } do |s|
      ...
      s.item :gamepatch_insights, safe_join([material_symbol('bar_chart_4_bars'), t('admin.gamepatch.groups.insights', default: 'Insights')]), nil do |i|
        ...
        if Rails.application.routes.url_helpers.respond_to?(:admin_gamepatch_research_index_path)
          i.item :gamepatch_research, safe_join([material_symbol('explore'), t('admin.gamepatch.research.title', default: 'Research')]), admin_gamepatch_research_index_path, highlights_on: %r{/admin/gamepatch/research}
        end
      end
