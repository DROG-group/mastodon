(() => {
  ...
  class GamepatchCardElement extends HTMLElement {
    connectedCallback() {
      ...
      this.data = this.parseDatasetJSON('data') || {};
      this.context = this.parseDatasetJSON('context') || {};
      this.state = this.parseDatasetJSON('state') || {};
      this.definition = this.parseDatasetJSON('definition');
      this.hostConfig = this.parseDatasetJSON('hostConfig');
    ...
    async render() {
      if (this.definition) {
        this.applyTheme(this.hostConfig || {});
        this.renderFromDefinition(this.definition || {});
        return;
      }
      ...
      try {
        ...
        this.renderFromDefinition(data.definition || {});
      } catch (err) {
